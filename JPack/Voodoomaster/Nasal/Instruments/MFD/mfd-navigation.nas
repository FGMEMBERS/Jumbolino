var nav = {
	...
	update : func {
		...
		if (getprop("/autopilot/settings/heading-bug-deg") != nil) {
			...
			if (yavail < math.abs(convfactor * me.range * (math.sin(slope / 57.2957795) / math.cos(slope / 57.2957795)))) { # The end of the line does not go on 																		till the end of the space given.
...
var get_elevation = func (lat, lon) {
	...
	if (info != nil) {
		var elevation = info[0] * 3.2808399;
	} else {
		var elevation = -1.0; 
	}
	return elevation;
